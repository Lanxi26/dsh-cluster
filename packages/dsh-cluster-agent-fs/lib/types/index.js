...
let ClusterAgentFsService = (() => {
    ...
    return class ClusterAgentFsService extends _classSuper {
        static {
            ...
            __esDecorate(this, null, _reportBinding_decorators, { kind: "method", name: "reportBinding", static: false, private: false, access: { has: obj => "reportBinding" in obj, get: obj => obj.reportBinding }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _openAgentSpace_decorators, { kind: "method", name: "openAgentSpace", static: false, private: false, access: { has: obj => "openAgentSpace" in obj, get: obj => obj.openAgentSpace }, metadata: _metadata }, null, _instanceExtraInitializers);
            __esDecorate(this, null, _syncGraph_decorators, { kind: "method", name: "syncGraph", static: false, private: false, access: { has: obj => "syncGraph" in obj, get: obj => obj.syncGraph }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        bindings = (__runInitializers(this, _instanceExtraInitializers), new Map());
        ...
        loadGraph() {
            try {
                ...
                if (data === null || typeof data !== 'object' || !Array.isArray(data.nodes) || !Array.isArray(data.edges))
                    return;
                ...
                this.graphEdges = data.edges.filter((rawEdge) => {
                    ...
                    return typeof edge.from === 'string' && typeof edge.to === 'string'
                        && (edge.direction === 'none' || edge.direction === 'single' || edge.direction === 'double');
                });
            }
            catch {
                // No persisted graph yet (first run): start empty and wait for syncGraph.
            }
        }
        /** Persist the synced canvas graph so it survives a `dsh web` restart. */
        async persistGraph() {
            const data = {
                nodes: [...this.graphNodes].map(([id, agentId]) => ({ id, agentId })),
        ...
        async create(id, name) {
            ...
            await writeFile(join(dir, 'agent.json'), JSON.stringify({ id, name, prompt: '', mode: 'any', spaceEnabled: false }, null, 2), 'utf8');
            return id;
        }
        /**
        ...
        async save(id, props) {
            const dir = this.dir(id);
            await mkdir(dir, { recursive: true });
            await writeFile(join(dir, 'agent.json'), JSON.stringify({ id, name: props.name, prompt: props.prompt, mode: props.mode, spaceEnabled: props.spaceEnabled }, null, 2), 'utf8');
            return id;
        }
        /**
        ...
        async listAgents() {
            ...
            try {
                ...
                // No cluster-agents directory yet: no classes to list.
            }
            return result;
        }
        /**
        ...
        classMeta(agentId) {
            try {
                ...
                // Missing or invalid agent.json is "no class metadata": callers fall back.
                return null;
            }
        }
        /**
        ...
        allowFlow(from, to) {
            if (from === to)
                return true;
            // No flow configured yet (empty canvas): open by default. Once edges exist,
            // deny-by-default applies and only declared flows pass.
            ...
            for (const edge of this.graphEdges) {
                ...
                if (edge.direction !== 'none' && a === from && b === to)
                    return true;
                if (edge.direction === 'double' && a === to && b === from)
                    return true;
            }
            return false;
        }
        ...
        async syncGraph(nodes, edges) {
            ...
            for (const node of nodes)
                this.graphNodes.set(node.id, node.agentId);
...
export { ClusterAgentFsService };
// A Service subclass is self-providing under its Context key (clusterAgentFs).
